...
Après 4 secondes (`HAL_Delay(4000)`), l'écran est effacé et remplacé par l'écran principal.
...
```
      WB BLE Sensor           ← ligne 0, centré
                              ← ligne 1, vide
Distance : 123 cm            ← ligne 2, mis à jour toutes les 500ms
```
...
- Mesure (`VL53L0X_PROXIMITY_PrintValue()`) :
  ...
  if (prox_value < 2000) {                        // seuil 2000 mm = 200 cm
      distance = prox_value / 10;                  // division entière mm → cm
      sprintf(distanceText, "Distance : %3d cm", distance);
      UTIL_LCD_DisplayStringAtLine(2, distanceText);
  } else {
      UTIL_LCD_DisplayStringAtLine(2, "Distance > 200 cm");
  }
  BSP_LCD_Refresh(0);
  ```

- La mesure single-shot est bloquante (~30–33 ms typique pour VL53L0X), exécutée dans la tâche séquentielle.
- La conversion mm → cm est une **division entière par 10** (pas par 10.0) : 15 mm → 1 cm (troncature, non arrondi).
- Le timer VL53L0X tourne en permanence (démarré via `VL53L0X_Start_Measure()` dans `APP_BLE_Init()`), y compris quand d'autres modes BLE sont actifs.

**Interactions avec les autres écrans :** Quand les notifications Motion ou Env sont actives (écrans 3/4), ces modes écrivent eux aussi sur la ligne 2 de l'OLED (à une fréquence plus élevée pour le Motion : 20 Hz). La distance continue de s'afficher brièvement toutes les 500 ms mais sera écrasée par le Motion à 50 ms, ce qui la rend visuellement invisible en mode Motion actif.
